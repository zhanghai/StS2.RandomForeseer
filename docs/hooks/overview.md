...
- `Common/Mirrors/ModelMethodMirrorRegistry.cs` handles single-model virtual-method dispatch: exact model-type registration, override detection, lookup caching, linked trace scoping, and unsupported-risk recording. Action registries keep explicit handled/ignored registrations separate from resolved Type lookups, expose a read-only `Query` that reuses lookup policy without creating a trace scope, executing a handler, or recording risk, and may register one Type-level inferer for unregistered gameplay overrides. Their `AllowInference` policy defaults to enabled; changing it clears only resolved lookups, so explicit registrations and the inferer remain intact. Inferred handlers are cached by exact runtime type and record incomplete rather than unsupported risk. Each dispatch frame stores the mapped source model and mirrored base method. Result registries require a handler that supplies the return value and do not currently support inference.
...
- Combat and out-of-combat code have independent `HookMirrors` facades but share the registry infrastructure. Mirrored model behavior that is not a hook, such as orb virtual methods, `CardModel.OnPlay` and `PotionModel.OnUse`, lives in its model domain under `Mirrors/` and follows the same facade/registry split.
