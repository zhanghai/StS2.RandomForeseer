...
- **预测卡牌打出触发效果**：战斗卡牌预测会模拟原版卡牌打出前后的支持效果，包括余像、劫难、钢笔尖、干瘪之手、缓慢等遗物与能力的连带格挡、伤害、抽牌、费用和生成牌变化；未完整支持且会影响结果的触发会显示偏移警告。
...
- **实验性卡牌预测**：战斗预测可通过通用规则识别更多未单独适配卡牌的攻击、格挡和抽牌行为，并显示这些卡牌触发的连带效果。默认关闭。
- **预测卡牌连锁效果**：战斗卡牌预测可显示由当前卡牌间接触发的支持卡牌和战斗效果，并提供因果说明。默认关闭。
...
- 送货员补充的卡牌、药水和遗物；大锅与星系仪会先计入拾起效果推进的随机数
